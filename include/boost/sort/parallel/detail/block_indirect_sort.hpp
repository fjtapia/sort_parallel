...
{   //-------------------------- begin -------------------------------------
    ...
    // The function launched with the futures is "execute the functions of the
    // stack until this->counter is zero
    // vbuf[i] is the memory from the main thread for to configure the thread
    // local buffer
    for (uint32_t i = 0; i < nthread; ++i) {
