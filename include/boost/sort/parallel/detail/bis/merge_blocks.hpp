...
struct merge_blocks
{
    ...
    {   //--------------------------- begin -----------------------------
        ...
        function_t f1 = [this, rng_input, &son_counter]() -> void {
    ...
    ///                      inside an object
    //------------------------------------------------------------------------
    void function_cut_range(const range_pos &rng_input, atomic_t &son_counter)
    {   //-------------------- begin -------------------------------------
        util::atomic_add(son_counter, 1);
        function_t f1 = [this, rng_input, &son_counter]() -> void {
...
{   //---------------------------- begin -----------------------------
    ...
    while (pos_ini < pos_last) {
        ...
        if (pos < pos_last) {
            in_place_merge_uncontiguous(bk.get_range(bk.index[pos - 1].pos()),
        ...
        if ((pos - pos_ini) > 1) {
            ...
            function_merge_range_pos(rng_aux, counter);
        };
        pos_ini = pos;
    };
        ...
        if (final or not mergeable) {
            ...
            if (rp_final.size() > 1) {
                if (rp_final.size() > Group_size) {
                    function_cut_range(rp_final, counter);
                }
                else
                {
                    function_merge_range_pos(rp_final, counter);
                };
            };
            posx_ini = posx;
            if (not final) {
