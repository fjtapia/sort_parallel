...
void test3 ( )
{
    ...
    bsort::parallel_stable_sort< iter_t, compare > (V3.begin ( ),
    		                                        V3.end ( ), 0);
...
void test5 (void)
{
    ...
    for (unsigned i = 0; i < NELEM; i++) A.push_back (my_rand ( ));
    B = A;
    ...
    for (uint32_t i = 0; i < A.size ( ); ++i) BOOST_CHECK (A[ i ] == B[ i ]);
};
...
void test6 (void)
{
    ...
    for (unsigned i = 0; i < NELEM; i++) A.push_back (NELEM - i);
