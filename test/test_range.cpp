...
/// @brief
///
/// @author Copyright (c) 2010 2015 Francisco Jose Tapia (fjtapia@gmail.com )\n
...
std::ostream &operator<<(std::ostream &out, range<iter_t> R)
{
    ...
    while (R.first != R.last) out << (*(R.first++)) << " ";
    out << std::endl;
    return out;
}
...
void test2()
{
    ...
    while (R2.first != R2.last) BOOST_CHECK(*(R2.first++) == 999);
};
//
//****************************************************************************
//                          TEST OF HALF_MERGE
...
void test3()
{
    ...
    for (uint32_t i = 0; i < 10; ++i) BOOST_CHECK(*(Rz.first + i) == i);
