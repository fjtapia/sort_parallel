...
GCC Parallel ( based on OpenMP )
    1. parallel::sort ( parallel ,not stable )
    2. parallel::stable_sort ( parallel , stable )
...
The GCC Parallel need OpenMP support in the C++ compiler
...
The result generated are in the file results_GCC.txt

Depending of the SW installed on your computer, you must select an script 
or another.

run_GCC.sh        = Boost Parallel + STL + Timsort+ GCC Parallel
run_GCC_BOOST.sh  = Boost Parallel + STL + Timsort+ GCC Parallel + Spreadsort
run_GCC_TBB.sh    = Boost Parallel + STL + Timsort+ GCC Parallel + TBB
run_GCC_TBB_BOOST = Boost Parallel + STL + Timsort+ GCC Parallel + Spreadsort + TBB
...
have the results in Results_GCC.txt
